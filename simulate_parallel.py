...
if __name__ == '__main__':
    ...
    # definte task inputs to pool map
    MAX_ITER = 20_000
    ABS_TOL = 1e-4
    tasks = [(u0, interior_mask, MAX_ITER, ABS_TOL) for u0, interior_mask in zip(all_u0, all_interior_mask)]
    ...
    #run Jacobi for each floor plan
    def process_jacobi(task):
        u0, interior_mask, max_iter, atol = task
        return jacobi(u0, interior_mask, max_iter, atol)

    pool = Pool(processes=num_process)
    
    all_u = np.array(pool.map(process_jacobi, tasks, chunk_size))
    print(all_u.shape)
